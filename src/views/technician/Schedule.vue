			...
			<div class="schedule-table" v-loading="listLoading">
				...
				<div v-if="tableData.length">
					<el-table :data="tableData" border style="width: 100%">
						...
						<!-- 技师结束 -->
						<!-- 7天时间 -->
							<el-table-column min-width="150" v-for="(item,index) in tableData[0].scheduleDateInfos" :key="index" :label="item.sevenDate+'('+weekNum(item.weekNum)+')'" align="center" className="work">
								<template scope="scope">
									<!-- 
										1：有工资时间
										2：没有工作时间但是有工资安排
										...
										<div class="work-add-bo">
											<div class="time-arrange-content">
												...
												<div v-if="scope.row.scheduleDateInfos[index].workBeginTime || ((scope.row.scheduleDateInfos[index].workBeginTime || scope.row.scheduleDateInfos[index].techScheduleInfos) && data.type!='holiday')" 
													...
													<span>{{data.type=='holiday'?'休假':(data.type=='order'?'订单':(data.orderType=='group_split_yes'?'(多次服务)':'(单次服务)'))}}</span>
												</div>
											</div>
										</div>
									</div>
...
<script>
	...
	export default{
		...
		methods:{
			...
			schedulePath(item){
				//判断是不是多次服务
				if(item.orderType == 'group_split_yes' || item.orderType == 'group_split_no'){
